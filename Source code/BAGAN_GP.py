...
!git clone https://github.com/GH920/improved-bagan-gp
...
random.seed(2021)
ratio=10
major_idx = np.where(labels == 7)[0] # majority sneaker
minor_idx = np.where(labels == 8)[0] # minority bag

sample_idx = np.random.choice(minor_idx.shape[0], minor_idx.shape[0]//ratio, replace=False)  
minor_idx = minor_idx[sample_idx]
print(major_idx.shape, minor_idx.shape)
idx = np.append(major_idx, minor_idx)
...
majority, minority = generate_images_binary(n_classes=n_classes, latent_dim=latent_dim, x_test=x_test, generator=bagan_gp.generator, steps=4, output_dim=[28,28])

print(majority.shape, minority.shape)

np.save("Fashion-MNIST-Majority", majority)
np.save("Fashion-MNIST-Minority", minority)
